  ...
  Widget build(BuildContext context) {
    return AppBaseScreen(
      appBar: AppBar(
        ...
        title: AppText(txt: 'Available Buses', size: 15, color: AppConst.white,),
      ),
      child: StreamBuilder(
    stream: Stream.periodic(Duration(seconds: 60))
        .asyncMap((i) =>
    _futureBusList = getBuses()), // i is null here (check periodic docs)
                ...
                itemBuilder: (BuildContext context, int index) {
                  ...
                  return FutureBuilder<String>(
                    ...
                    builder: (BuildContext context, AsyncSnapshot<String> snapshot) {
                      if (snapshot.connectionState == ConnectionState.waiting) {
                        return busesShimmerLoading(width: MediaQuery.of(context).size.width, height: 70, borderRadius: 0);
                      } else if (snapshot.hasError) {
                        return Center(child: Text('Error: ${snapshot.error}'));
                      } else if (snapshot.hasData) {
                        ...
                        return Padding(
                          ...
                          child: AppListTile(
                            ...
                            onTap: () {
                              Navigator.pushNamed(context, RouteNames.exploreBuses, arguments: {
                                'id': bus.id.toString(),
                              },);
                            },
                          ),
